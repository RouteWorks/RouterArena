...
def test_lever_3_pattern_is_detected(tmp_path: Path) -> None:
    ...
    current = [
        ...
        {"global index": f"Q{i}", "prediction": ("model_b" if acc == 0.0 else "model_a")}
        for i, acc in enumerate([1.0, 1.0, 0.0, 0.0, 0.0])
    ]
    ...
    assert errors, "Lever #3 pattern (100% reassignments target accuracy=0) should be flagged"
    joined = "\n".join(errors)
...
def test_detects_accuracy_to_prediction_pattern(tmp_path: Path) -> None:
    ...
    assert errors, "Lever #3 code pattern (accuracy check → prediction write) should be flagged"
...
def test_accuracy_read_without_prediction_write_passes(tmp_path: Path) -> None:
    ...
    report_script.write_text(textwrap.dedent("""
        import json
        preds = json.load(open('predictions.json'))
        for p in preds:
            if float(p.get('accuracy', 0) or 0) == 0.0:
...
def test_self_skip(tmp_path: Path) -> None:
    ...
    fake_self.write_text(textwrap.dedent("""
        for p in preds:
            if float(p.get('accuracy', 0) or 0) == 0.0:
...
def test_full_check_catches_lever_3_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--predictions", str(pred_dir / "llm-router.json"),
        "--baseline", str(pred_dir / "llm-router.json.bak.honest"),
...
def test_full_check_passes_on_clean_state(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", [
        "check_submission_integrity.py",
        "--predictions", str(pred_dir / "llm-router.json"),
        "--scripts-dir", str(tmp_path / "scripts"),  # doesn't exist - OK
    ])
